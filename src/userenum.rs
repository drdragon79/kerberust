...
pub fn start(args: UserenumArgs) {
    ...
    if args.username.userlist.is_some() {
        let file = File::open(args.username.userlist.unwrap()).unwrap_or_else(|e| {
...
fn check_user(dc: &Kdc, username: String, npauth: bool, opsec: bool) {
    ...
    if !opsec {
        ...
        asreq
            ...
            .req_body
            .etype
            .insert(0, encryption::rc4_hmac as i32);
    }
    ...
    match asreq.send_and_parse(dc) {
        ...
        Ok(asrep) => {
            println!(
                "{P}Valid User: {} - No pre-auth required!",
                username.green()
            );
            if npauth {
        ...
        Err(resp_error) => {
            ...
            let error = krberror.geterrorvalue().unwrap();
            if err_code == krberrors::KDC_ERR_WRONG_REALM {
                println!("{M}Wrong Domain! Exiting!");
                process::exit(1);
            }
            let valiusererrorcodes = [krberrors::KDC_ERR_PREAUTH_REQUIRED];
